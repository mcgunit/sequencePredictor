...
All of them now also agree on **which Keno subset sizes to play**: every model asks `getKenoSubsetSizes` for the hyperopt-tuned `use_5`..`use_10` flags. The DL rows previously hardcoded `range(5, 11)`, so they placed a bet at all six sizes while every statistical model played only the tuned ones (currently just `use_10`) — making those rows' tracked results non-comparable with the rest. `HyperoptDeepLearning.py` reads the same tuned choice via `tuned_keno_subset_sizes`, falling back to all six only when a game has no tuned flags at all, so its profit signal never silently becomes zero. Three further rows come from the foundation-model track (roadmap item 5): **`Chronos Model`** and **`TimesFM Model`**, two pretrained time-series models from different labs asked zero-shot for each drawn position's next value, and **`OrderStatistics Baseline`**, the historical mode of each position - the yardstick both of them have to beat, since a sorted game's position 1 is its minimum and therefore predictable in distribution without the draw being predictable at all.
...
`HyperoptStatistics.py` uses Optuna to tune each statistical model's parameters per game, driven by `src/Backtester.py`, which evaluates each model using rolling historical validation: for every historical draw, the model is trained only on previous draws and compared against the next real result. A run's best parameters are written to `bestParams_<game>.json` when they pass the champion/challenger gate described below (*The objective and the gate*), together with the served row's score (used by `Predictor.py` at prediction time), including a `modelScores` entry per model (the score of the parameters actually served after the gate - this run's best trial on a replace, the re-scored incumbent on a keep - measured on this run's window and objective, and removed when the row could not be scored this run; keyed by the same display name `Predictor.py` and the report use).
...
`HyperoptBoost.py` does the same job for the boosting model, and is now a direct mirror of `HyperoptStatistics.py` rather than the separate legacy pipeline it used to be. Two run-time guards keep it inside a weekend: **CatBoost's tree depth is searched over 1-7** (its symmetric trees cost 2^depth - measured median minutes per trial by depth: 1-6 → 1-4, 8 → 24, 10 → 64, with single trials up to 12 hours, while XGBoost/LightGBM keep the full 1-10 range), and **every trial has a wall-clock budget** (`--trial-timeout`, default 1200 s) after which the Backtester terminates its worker pool and Optuna records the trial as pruned. Every trial is also stamped with a monotone **fit-cost proxy** (estimators x window, x 2^depth for CatBoost), and a later configuration at least as expensive as one that already hit the budget is skipped immediately as a predicted timeout (recorded as pruned, not counted toward `--trials`). This matters because Optuna's TPE sampler proposes its first ten trials at random and on keno roughly half the search space cannot finish one refit block inside the budget: the per-position formulation trains 20 multiclass models over 80 numbers, and XGBoost/LightGBM grow one tree per class per round, so keno LightGBM burned six 20-minute timeouts in its first seven trials, each more expensive than the one that finished. The gate only trusts timeouts recorded under the same budget tag (machine, window, cadence, budget), so stronger hardware or a larger budget starts from a clean slate. Two further levers attack the real cost driver - refitting a boosted ensemble for each evaluated day (90 by default since September 2026, 31 before): a **refit cadence** (`--refit-every`, default: the window over five blocks - 7 at 31 days, 18 at 90 - so a trial keeps the shape the budget, thread count and parallelism below were calibrated for) trains one fit per block of consecutive days (block-aligned on the data *before* the oldest day of the block, so it is conservative, never leaky; every day still builds its own prediction features, and the fit cache keeps the main and special-column fits side by side) - identical for every trial, so the ranking is untouched while production keeps refitting daily. Measured on eurodreams CatBoostMultiLabel (2 trials x 21 days): 7.5 min at cadence 1 -> 66 s at cadence 7, with objective values in the same range. Threads are per library, from measurement: CatBoost scales (one fit: 4 threads = 3.1x) and gets the cores its fewer workers leave free; XGBoost/LightGBM stay single-threaded (giving XGBoost 5 threads made a study 5x *slower*), and the tuner pins OpenBLAS/OpenMP to one thread per process because every worker otherwise inherited a 16-thread BLAS pool whose spin-waiting cost ~10 cores of pure overhead per fit; and **conservative pruning** (`--prune-percentile`, default 25, 0 disables): the partial score after each completed day is reported to an Optuna `PercentilePruner` that stops only trials in the bottom quartile after half the window and only once five trials have completed - pruned trials are recorded as pruned, never as bad scores. **Trials of one study run in parallel processes** (`--parallel-trials`, default auto = the cores the per-trial workers leave idle: with 5 refit blocks on 15 cores, 3 trials at a time - the block count the default cadence keeps at any window of 29 days or more; below that the 7-day floor gives fewer blocks and more concurrent trials) for the XGBoost/LightGBM strategies, whose single-threaded fits used 5 of 15 cores; CatBoost keeps one trial at a time and the idle cores as fit threads. A further trial only launches while the machine keeps `--memory-reserve-gb` (default 2) available after it, with the expected footprint measured on the trials already running, and under 1 GB available the youngest trial is stopped and recorded as pruned rather than inviting the OOM killer. Each trial runs in its own process (the Backtester passes worker state through a module global, so two trials cannot share one), which also means a crashing trial no longer aborts the remaining strategies of a game, and trials left RUNNING by a killed run are marked failed when the study is reopened; the sampler runs with `constant_liar` so concurrent trials do not sample the same neighbourhood. Honest note: with a refit cadence above 1 all blocks fit concurrently, so pruning saves little compute (it mainly pays off at cadence 1); it is kept because it is free and protects the cadence-1 path. CatBoost's `border_count` (split candidates per feature) is exposed as a knob - `--catboost-border-count` (default 254, CatBoost's own), written to `bestParams_<game>.json` as `catBoostBorderCount`/`catBoostMlBorderCount` so `Predictor.py` serves the same value - for experimentation only: measured on this data (integer features with at most 50 distinct values) 32 vs 254 made no difference in either fit time (1014 s vs 1016 s for a 50-classifier multi-label fit) or ranking (Spearman 1.0), so the default is left untouched. Concretely: it evaluates through `src/Backtester.py` (one Optuna study per game+strategy, `load_if_exists=True`, walk-forward over the last `--days` draws) instead of the old rebuild-a-JSON-cache-and-total-the-profit loop; scores trials with the shared `score_from_summary` (since September 2026 the capped, lower-bound objective of `src/TuningScore.py` - see *The objective and the gate* below; before that profit per bet where a payout table exists, avg hits otherwise) rather than raw total profit; bets the game's served Keno subset sizes (the global `use_<n>` flags, as `Predictor.getKenoSubsetSizes` does) rather than the per-model inclusion mask it used to search; and merges its results into `bestParams_<game>.json` including a `modelScores["XGBoost Model"]` entry, so the boosting model's vote is weighted in `WeightedEnsemble Model` like every other model's. Its `STRATEGIES` / `STRATEGY_DISPLAY_NAMES` tables have the same shape as the statistical ones, so adding a second boosting method (LightGBM, CatBoost) is a one-entry change. The old version could not run at enough strength to be high-confidence prediction.

**The objective and the gate (`src/TuningScore.py`, `src/TuningGate.py`) - September 2026.** Two findings from the served parameters of that month changed how the statistical and boosting tuners score and write. First, the objective. Until then a trial's value was its raw profit per bet over a 31-day window (average hits where a game has no payout table), and on 31 days that number is decided by whether one jackpot-tier payout fell inside the window: the keno LightGBM parameters served from 10 Sept scored 5.0 per bet, of which one draw - a 6/6 with its nested 5/5, +348 EUR - was everything and the other 60 bets lost 38 EUR; the pick3 XGBoost and CatBoost winners were single straights (+756 and +676 EUR) on windows that otherwise lost, and the pick3 statistical studies' best values (21 and 42 per bet) are one and two straights. Optuna's per-day intermediate values show it directly: the cumulative score of each of these trials was negative for most of the window and jumped on one day. The repeat-and-average per trial the tuners once had (three repeats in the original `Hyperopt.py`, a mean-minus-std variant in August 2025 that was reverted a week later) would not have helped - the Backtester is seeded per day, so a repeat returns the same number; the luck is in the draws of the window, not in the model's randomness. Trials are now scored by a **lower confidence bound over the days** (mean − std/√days, the estimator `HyperoptEnsemble.py` already used): for keno the per-day profit per bet with every single bet's profit **capped at the lucky-strike threshold (20 EUR net)**, so a jackpot counts as one good bet and the 1-10 EUR tiers keep their value; for the positional games the per-day **slot hits**, which every draw informs - pick3's digits in the right slot, Joker+'s leading and trailing runs; for pick3 counted by `TuningScore` from the drawn-order ticket and draw the Backtester stores, because the Backtester's own pick3 `_hits` column is the historical *set* count that scores a right digit in the wrong slot (a box) like a straight; for Joker+ the `_hits` column, already L + R runs - with the capped profit bound as a 0.001-weighted tie-breaker - almost every pick3 window is a flat loss once a straight is capped, so profit alone would rank noise; for the games without a payout table the hits bound. The pruning callback scores the finished days with the same function, so partial and final values agree. The raw profit per bet and the number of lucky strikes travel with every trial as the user attribute `tuning`. The default window is **90 draws** (`-d`, was 31) for both tuners. For the boosting tuner the refit cadence scales with it (five blocks at any window of 29 days or more, see above), so a boosting trial's cost, its 20-minute budget and the trial parallelism are unchanged and its run grows only by the reference evaluations, up to two backtests per strategy and game. The statistical tuner has no cadence: each of its trials, and the Keno subset-tuning precompute, backtests every day of the window from scratch over the 15 workers, so both take roughly twice as long as at 31 days - seconds per trial still, and the precompute is now bounded at three trial budgets (an hour), beyond which the row is skipped for that run. Second, the write. Every run used to write `study.best_params` - the best trial the study had *ever* seen - which with the old objective was the trial that caught the biggest payout, kept until a later window happened to beat it (the keno 5.0 was never approached again: 2.13, 1.26). Now **this run's best completed trial is the challenger**, and it replaces the served parameters only if it beats **the incumbent** (the parameters in `bestParams_<game>.json`, re-scored on this run's window with this run's objective) **and the untuned defaults** (what `Predictor.py` serves when the keys are missing - `src/BoostingBase.py`'s table for the boosting rows, `HyperoptStatistics.SERVED_DEFAULTS` for the statistical ones), by `--gate-margin` (default 0). A trial without a finite score (a backtest that produced nothing) is never a challenger, gate or no gate. The references are scored by the strategy's own Optuna objective driven by an `optuna.trial.FixedTrial`, so the incumbent goes through exactly the code that scored the trials, each in a forked child (`HyperoptRunner.run_isolated`) because the coordinator must never run a fit itself before forking the next study's trials. A reference that cannot be scored (timeout, error) cannot block, and says so. Every decision is recorded under `bestParams_<game>.json["tuningGate"][row]` - challenger, incumbent and default scores, each one's diagnostics (raw profit per bet, lucky strikes, days), the window, the study's all-time best for context - and `modelScores` carries the score of what is actually served; a row the gate could not score this run loses its old raw-profit entry rather than keeping a weight measured on another scale. `--no-gate` writes the run's best without the comparison; nothing writes the study's all-time best any more. The Keno subset-tuning strategy uses the same capped bound. Two honest notes: a robust objective ranks candidates, it does not make them good - on the September data every keno and pick3 trial's bound is negative, as a fair ticket's would be; and the per-model Keno subset flags the tuners used to search (`lightgbm_use_5`, `markov_use_6`, ...) were written to the file but never read at serve time - `Predictor.getKenoSubsetSizes` uses the global `use_<n>` flags for every row - so a trial's value depended on ticket sizes that never reached production, and the gate would have compared unlike tickets. Trials and references now bet the served sizes (`served_keno_subsets`); the stale per-model keys in older files are ignored. The Keno subset-tuning precompute, the one step no trial budget reaches, is bounded at three trial budgets and the row is skipped for the run when it does not finish.
...
### Null controls, and what the best row scores on nothing (implemented)

The two controls above stopped being a plan on 2026-09-21. `src/ControlHistories.py` builds them - **synthetic** (draws generated under the game's own rules, no structure of any kind) and **shuffled** (the real draws reordered, so every marginal survives and only time order dies), both on the real calendar - and two scripts use them:

```
    python3 NullControls.py -g lotto -m both -n 3 -d 120
    python3 RandomnessDiscrimination.py -g lotto,pick3,euromillions -w 10 -n 5 --draws 1000
```

**`NullControls.py` (Q0): how good does the BEST row look on data with nothing in it?** Every ranking on the History page is a maximum over rows, and the maximum of N noisy estimates sits above their mean even when every row is worthless. Measured on lotto, 120 days x 3 seeds, eight model rows on their untuned defaults:

| control | average row | best row | selection gain | best-to-worst spread | random baseline |
|---|---|---|---|---|---|
| synthetic | 0.781 | 0.883 | **+0.102** | 0.219 | 0.794 |
| shuffled | 0.821 | 0.964 | **+0.143** | 0.289 | 0.794 |

(hits per draw; chance is 6²/45 = 0.8, and the measured random baseline of 0.794 is the apparatus confirming it measures what it claims.)

Three things to read off this. **A different row won almost every run** - PoissonMonteCarlo, XGBoost, Markov on the synthetic histories, PoissonMonteCarlo and PoissonMarkov on the shuffled ones - which is what a leaderboard looks like when there is nothing to lead. **The shuffled control is the harder bar**, not the softer one: it preserves whatever the real history's number frequencies happen to be, and the frequency-shaped models exploit that, so rows score higher there (0.821 average against 0.781) and selection buys more (+0.143 against +0.102). And therefore, on lotto, **a tracked row is not interesting until it beats about 0.96 hits per draw** - not the 0.8 of chance - because that is what the best of eight rows reaches on draws that are known to carry no signal at all.

One thing that looked like a bug and was not: the same winning row reached exactly the same score in two independent control histories (PoissonMonteCarlo at 0.9417 on synthetic seed 1 and shuffled seed 1; PoissonMarkov at 0.975 on two shuffled seeds). Every other row differed substantially in the same pairs - hits over 120 days are multiples of 1/120, so a collision among eleven rows is unremarkable. Worth recording because "identical number across two runs" is otherwise exactly what a silently reused dataset looks like.

**`RandomnessDiscrimination.py` (Q2): can anything tell real draw windows from fair simulated ones?** Windows of ten draws become multi-hot counts (per slot for pick3 and Joker+, so digit order survives), and the suite the design asks for - logistic, RBF SVM, random forest, gradient boosting, small NN, quantum kernel, VQC - is trained on a chronological split. Three comparisons run through the identical pipeline, because one number cannot be read on its own: *real vs synthetic* is the question, *synthetic vs synthetic* is the null band (two fair histories, true answer provably 0.5), and *shuffled vs synthetic* is the marginals control (same frequencies, no time order). Result over 1000 draws and 5 repetitions, as the best AUC of the suite per repetition:

| game | real vs synthetic | null band | shuffled vs synthetic | verdict |
|---|---|---|---|---|
| lotto | 0.588 ± 0.049 | 0.565 ± 0.076 | 0.582 | within the null band |
| euromillions | 0.551 ± 0.050 | 0.544 ± 0.030 | 0.536 | within the null band |
| pick3 | 0.495 ± 0.043 | 0.540 ± 0.045 | 0.534 | within the null band |

**No classifier, classical or quantum, separates real draws from correctly simulated fair ones beyond what the same pipeline scores on two histories that are identical in law.** That is the honest negative result this phase existed to produce, and it is the one the rest of the project should be read against.

Two methodological lessons came out of building it, both worth keeping:

* **The shuffled control was wrong first, and its own null band caught it.** Permuting the whole 1995-2026 history and then taking its newest 600 draws samples every rule era the game ever had: per-number counts came out with a standard deviation of 13.5 against the real 9.1 (chi-square p < 0.0001), so the control no longer shared the marginals it exists to preserve - and it "separated" better than the real history. The window is cut *before* the shuffle now, and the two match exactly.
* **The verdict itself fell into the trap it was built to expose.** Comparing the best AUC over seven classifiers and five repetitions against the *mean* of the null comparison's individual runs made euromillions read as evidence (0.637 against a 0.598 threshold). Best-of-seven is biased upwards even when all seven are worthless, so it has to be held against the null's own best-of-seven: 0.551 ± 0.050 against 0.544 ± 0.030, which is nothing. `--reanalyze` recomputes the verdicts of saved records under the current rule, so a corrected rule can be applied without paying for the run twice.

Limits, stated because they bound what the numbers mean: the models run on untuned defaults, so this measures selection over rows and not the selection over hyperparameters that a full hyperopt on a control history would add (that is the remaining Q0 item, and it makes these figures a lower bound); a synthetic history reproduces today's rules over the whole calendar, so the default window is restricted to the most recent draws where the rules are stable; and the foundation rows are excluded unless `--foundation` is passed, because each control history would need its own forecast pass.
...
### 5. Foundation models as per-position predictors (medium) - done (zero-shot, two models)

Done 2026-09-20/21 as decided: zero-shot, no fine-tuning. Every drawn position is one univariate series, `amazon/chronos-2` is asked for the next value's distribution, and that distribution over the game's labels is the per-position score shape the positional consumers already understand - so it serves as the `Chronos Model` row (`src/ChronosModel.py`) with `run()`, `score_positions()` and `score_numbers()` like any other model, including the main/special split through `run_model_with_special_column`. **TimesFM-3** (`google/timesfm-3.0-pytorch`, `src/TimesFmModel.py`) followed as the second entry through the identical protocol: both are thin subclasses of `src/FoundationModel.py`, which owns the worker, the cache and the ticket construction, so the two models differ only in which checkpoint answers. That is the point of having two - they are different models from different labs trained on different corpora, so what they agree on is a property of the data and what only one of them sees is a property of that model.

**It runs outside the pipeline's interpreter, and that was not optional.** The foundation stack pulls numpy 2.x; the pipeline runs TensorFlow 2.16 on numpy 1.26. Installing it into the system interpreter would have upgraded numpy underneath the daily predictor. The libraries therefore live in their own directory (`/root/.foundation-libs`, `FOUNDATION_LIBS` to move it) that only a small worker process puts on its path (`src/foundation/chronos_forecast.py` and `timesfm_forecast.py`, JSON lines over stdin/stdout, sharing `quantile_labels.py`). The worker is kept warm for the life of the model object, because a backtest asks for one forecast per day; it is launched with `PR_SET_PDEATHSIG` and an `atexit` close, since the history rebuild runs the statistical step in a pool of spawned processes and eight orphaned 0.4 GB workers is how a 16 GB box dies quietly. Missing libraries, a crash or a timeout disable the row with one log line and produce no ticket, which every caller already treats as "this model produced nothing".
...
```
    ...
    python3 -m pip install --target /root/.foundation-libs --no-deps timesfm==3.0.2
```
...
(`timesfm` needs `--no-deps` only because its torch extra would re-resolve the CPU wheel; numpy, huggingface_hub and safetensors are already there from the first two.)

**Measured** on this box, CPU only:

| | Chronos-2 | TimesFM-3 |
|---|---|---|
| first run (downloads weights) | ~460 MB | ~1.2 GB, 6 min |
| warm load | 1.4 s | 4.2 s |
| six lotto positions | 0.16 s | 0.41 s |
| a further point in time | 0.28 s | 0.40 s |
| resident memory | 0.84 GB | 2.84 GB |
| quantile grid published | 21 levels (0.01-0.99) | 9 (the deciles) |

Both are cheap enough to run for every game every day without touching the GPU or the deep-learning budget. **They run on the live day only** (`skipRows == 0`), never inside a history rebuild: that rebuild runs the statistical step in a pool of up to eight spawned children, and a worker in each would add 0.8-2.8 GB apiece on top of their TensorFlow imports. It is also the honest choice - backfilling a new row into days that are already scored hands it a track record it never earned out of sample, the same rule the tuned `SubsetEnsemble Model` follows.

**How to read the row, and the baseline that makes it readable.** For the sorted set games the per-position series *are* order statistics: position 1 of lotto is the minimum of six draws from 1-45, so a forecast of "about 5" is arithmetic, not prediction. Chronos reproduces exactly that - its six lotto positions peak at 2, 11, 18, 25, 35, 44 - which is why the row is meaningless against chance and is instead served next to a new `OrderStatistics Baseline` row (`Baselines.ColumnFrequencyBaseline`): the historical mode of each position, computed from the same data through the same interface, so the History page ranks them side by side. A per-position model is interesting only once it beats that row. For the positional games an honest process gives near-uniform forecasts, which is what happens - and the repeated-digit tickets that follow (`[9, 9, 9]` on 2026-09-20, `[8, 8, 8]` on 2026-09-21) are that result rather than a fault. Measured on 2026-09-21, Chronos gives digit 8 a mass of 0.121 / 0.126 / 0.128 across pick3's three positions against 0.083-0.089 for the least likely digit, a most-to-least ratio of 1.4-1.5, and the same digit tops all three slots because all three series carry the same recent excess: over the last 100 draws 8 was the most frequent digit in positions 1 and 3 (0.160 and 0.180) and 9 in position 2. That excess is what chance looks like: over a 100-draw window a digit's frequency has a standard deviation of 0.030, so 0.160 and 0.180 are 2.0 and 2.7 deviations, and they are the largest of the 30 digit-by-position cells, where the expected largest is about 2.2. Over the full 512 draws the same digit sits at 0.104 / 0.100 / 0.113, inside one deviation of uniform (0.013). The model is tracking recency noise, which makes it a recency-weighted frequency estimator on these games and puts it in the same class as the baseline it is served beside. The *scores* are therefore the output worth reading there, not the ticket, and a deviation both large and stable across weeks would be the finding. The quantile grid is not involved: the model's 21 native levels, 199 levels inside its trained range and 199 that reach past it all give the same distribution to three decimals, so the worker now asks inside the trained span only.

**What the second model was worth, immediately.** TimesFM-3's first lotto forecast returned 1 as the most likely value for four of the six positions, each at exactly 0.100 - a number that cannot be data. The cause was in the shared step that turns a quantile curve into label probabilities, not in either model: the curve was read as a CDF clamped at its outermost levels, and whatever mass lay outside was folded onto the first and last label. With Chronos-2's 0.01/0.99 levels that fold moves 0.005 per side and changes no argmax, which is why it survived review; with TimesFM-3's deciles it dumps a flat 0.100 on the lowest label of every series. The curve is now anchored at the bounds of the game's own support - the points (first label - 0.5, 0) and (last label + 0.5, 1) are added before interpolating - so each tail is spread uniformly over the stretch it actually occupies, which is the maximum-entropy reading of "this much mass is out there somewhere" (`src/foundation/quantile_labels.py`). Chronos's published numbers are unchanged to three decimals; TimesFM's became sane. A second implementation of the same idea found a defect in the first one's reading within an hour, which is the argument for having it.

**The two models, side by side** (2026-09-21, lotto, same 512 draws): Chronos 2, 11, 18, 25, 35, 44 against TimesFM 2, 11, 21, 25, 40, 44 - agreement on four of six positions and the same shape everywhere, which is what two honest order-statistics readings should look like. On pick3 they disagree outright - on the same 512 draws Chronos read `[8, 8, 8]` and TimesFM `[9, 3, 9]`, and on the first day both rows were actually served (2026-09-21, after that day's draw landed) Chronos read `[8, 1, 8]` against TimesFM's `[9, 9, 9]` and TimesFM's spread is wider (ratios 1.5-2.6 against 1.4-1.5), but on nine knots that width is as much resolution as signal. Two models built by different labs producing different "most likely" digits from the same history is the cleanest statement available that there is nothing there to find.

**As a meta-learner feature.** `Chronos Model` is now a base-model column in `src/ModelFactory.py` (`BASE_MODEL_NAMES`), appended last for the same reason `XGBoost Model` was: every artifact stores its own `feature_names` and never asks for a column it was not trained with, so older artifacts keep working and the existing column order is untouched. `TimesFM Model` is available the same way behind `"useTimesFmFeature": true` - off by default, because 2.8 GB for a second reading of the same kind of signal is a choice, not a default. The feature only exists after a retrain: `TrainMetaLearner.py` recollects automatically, because the score-table cache is now invalidated by the base-model *set* and not only by their parameters.

Getting them into that table needed one piece of machinery. `src/Backtester.py` forks its worker pool and shares the model objects copy-on-write, and a forked child must neither talk to the parent's worker (two conversations in one pipe) nor start its own (fifteen children x 0.8-2.8 GB). So `ModelFactory.prepare_foundation_scores` forecasts every day the backtest will ask for up front, in the parent, with one warm worker (measured 0.51 s per lotto day, forecast plus the data reload, so about 2.5 minutes for a 300-day window), then closes it and switches the model to cache-only; the children read the inherited dictionary and start nothing, and a day nobody precomputed returns "no signal" rather than a surprise 0.9 GB. `python3 src/FoundationModel.py <game>` is the self-check for exactly that: it precomputes, forks four children, and asserts they served from the cache and started zero workers. Verified end to end on pick3: the collected table carries `Chronos Model_position_scores`, the trained artifact's features are Markov, MarkovMonteCarlo, PoissonMonteCarlo, LaplaceMonteCarlo, XGBoost, Chronos, and a process scan through the whole collection saw a peak of **one** forecasting worker - the parent's - rather than one per pool child. The first attempt at this got it wrong in a way worth recording: the models were recognised with `isinstance`, which silently returns False here because `src/` modules are imported both as `src.X` and as `X`, so two copies of the class exist. The precompute did nothing, the pool spawned a worker per child, and the only symptom was that it worked. Identity is an attribute now (`IS_FOUNDATION_MODEL`), and `_start_worker` refuses outright when the model is used in a process other than the one that built it.

An adversarial review of the finished work then found six more defects, all fixed, of which three mattered:

* **The column was trained on real scores and served as 0.0.** `Predictor.py` builds a meta-learner's feature vector from the artifact's own `feature_names`, filling 0.0 for any feature it has no instance for - and the foundation models had not been added to that instance map. A retrained artifact would have asked for a Chronos column and silently received zeros. Demonstrated after the fix by serving pick3 twice, once with the instance and once with it suppressed: `MetaLearnerV2` answers `[1, 5, 8]` against `[0, 7, 8]`. The lesson is about the convention rather than the typo - "absent feature means 0.0" turns a wiring mistake into a wrong answer instead of a crash.
* **One unusable day killed a whole game.** `precompute()` had no per-key guard, while the Backtester tolerates exactly the same failure per model per day. With the default 300-day window, eurodreams (exactly 300 stored draws) asks for a slice that leaves no rows at all and `Helpers.load_data` raises, so that game would have lost its meta-learner entirely. Reproduced, then fixed with a per-key `except` and a `try/finally` that releases the worker whatever happens - without it a mid-run failure left 0.8-2.8 GB resident for the rest of the run.
* **A verdict that outlived its cause.** `_unavailable` was set on any worker failure and never cleared, and `Predictor` closes the worker between games - so one transient timeout on the first game would have dropped the row for every game after it. `close()` now re-arms; the refusals that must stay permanent (missing libraries, a model carried across a fork, cache-only) are re-checked on every call anyway.

The other three: the Chronos worker was feeding possibly-crossing quantiles into an interpolation that requires a monotone curve (now sorted after the clip, which also makes the docstring's precondition an enforced one); `foundationContext` was not part of the score-table cache fingerprint, so a changed context could have been trained against a stale table; and exact argmax ties - common, because a piecewise-linear CDF is flat across stretches of labels - always resolved to the lowest label, a measured bias of about 0.8 of a label, now broken toward the slot's own mean. None of the documented numbers above changed as a result: they were re-measured after every fix.

**Flags**, all in `bestParams_<game>.json`: the three rows are on by default (`"useChronos"`, `"useTimesFm"`, `"useOrderStatisticsBaseline"`), the Chronos meta-learner column is on by default (`"useChronosFeature"`), the TimesFM one is off (`"useTimesFmFeature"`), and `"foundationContext"` (default 512) is the number of past draws either model is shown. On a machine without the libraries every one of them degrades to nothing with a single log line.

Nothing is open in this item. What it leaves for later, if the tracked results ever justify it: fine-tuning rather than zero-shot (which the decision above deliberately ruled out until a zero-shot row proves interesting), and a third checkpoint through the same protocol, which is now a subclass and a worker script rather than a project.
...
- **Q0 classical controls - the two benchmarks are built (2026-09-21).** `NullControls.py`, on `src/ControlHistories.py`, runs the production evaluation path against two control histories per game and reports what the *best* row scores when there is provably nothing to find - see *Null controls* below for the measurements. Previously done: `TCN.py` is tunable and runs as `TCN Base Model`, and the daily run has `--ai` on again, so TCN accrues a production track record. Still open in Q0: remove or wire the dead `num_lstm_layers` setting (stored and written to metadata, never used by `create_model`); re-run the **hyperopt** itself on a control history, which measures the selection effect of tuning on top of the selection over rows the benchmark measures today; persist per-number **calibration and ranking metrics** (Brier / log-loss, precision at draw size, reliability bins) instead of the printed accuracy/AUC line in `fit_meta_model`; and reserve an untouched chronological **lockbox** that `TrainMetaLearner.py`'s 80/20 and `HyperoptQuantum.py`'s 75/25 splits never see until designs are frozen.
...
- **Q2 randomness discrimination - built and run (2026-09-21), first answer: no.** `RandomnessDiscrimination.py` turns windows of draws into multi-hot counts (per slot for the positional games), trains the full suite the design asks for - logistic, RBF SVM, random forest, gradient boosting, small NN, quantum kernel, VQC - on a chronological holdout, and repeats over independent synthetic histories. Across lotto, pick3 and euromillions (1000 draws, 10-draw windows, 5 repetitions) the real-versus-synthetic separation sits inside the band the same pipeline produces on two histories that are identical in law. Details and numbers in *Null controls* below. What remains open: other window lengths and feature families, the remaining games, and a rule-change map so a longer window can be used without the rule history contaminating the answer.
...
Built 2026-09-20/21 in two halves. First the services (`services.js`): the Council API is supervised and shown on the admin **Jobs** page, which established the status, logging and page shape. Then the schedule itself (`jobs.js`, see *The schedule*): seven jobs - the daily predictor and the six of the weekly chain - a single-worker FIFO queue that waits on `process.lock` instead of exiting, the chain triggered by Saturday's predictor finishing, catch-up for a slot missed while the box was down, `setsid --fork` launches verified to come up with PPID 1, adoption of a job that outlived a server restart (measured: a restarted server picks its running job back up and still records the right duration and exit code), rotation of `log/*.log` at 10 MB, and a run record carrying the git SHA. `test/jobs.test.js` covers the schedule arithmetic and fails if `jobs.js` and the shell scripts ever disagree about what runs in which order.
...
A service is an ordinary child on purpose: it is cheap to restart and must simply always be up, so it should come and go with the server. The scheduled pipeline jobs are the opposite case - a twelve-hour tuning run must survive a deploy - so they are launched differently, by the module described next, and rendered above the services on the same page.

### The schedule (`jobs.js`)

The daily predictor and the weekly tuning chain are the server's own work now, not crontab's (roadmap item 8). Seven jobs: `Predictor.py -a true` at 09:00 every day, then `HyperoptStatistics.py`, `HyperoptBoost.py`, `HyperoptRLTicket.py`, `HyperoptEnsemble.py`, `HyperoptQuantum.py` and `TrainMetaLearner.py` as the weekly chain - the same commands, the same order and the same log files as `runPredictor.sh` and `runHyperopt.sh`, which stay as the hand-run path (`test/jobs.test.js` fails if the two drift apart).

What it does differently from cron:

- **The chain starts when Saturday's predictor finishes**, not at 14:00. That is the 3.5 idle hours of 2026-09-19 gone. The trigger is observed either from the scheduler's own run or from `process.lock` being released by a predictor someone else started, so it works during the dry run too.
- **A job that finds the pipeline busy waits** in a single-worker FIFO queue instead of exiting - the failure mode that could skip a week of tuning in silence. The same FIFO is why a Saturday chain running into Sunday 09:00 queues that day's predictor behind it rather than pre-empting the tuner, as agreed.
- **A missed slot is caught up** for 20 h (so a box that comes back at 04:00 still runs the day it missed), and the chain until Monday 09:00. The first ever start records the slots in its window as already handled instead of firing them - they belong to the crontab era.
- **A running job survives a deploy.** Every job is launched with `setsid --fork`, so its parent is PID 1 and pm2's `treekill` walks past it; the restarted server finds the run in `config/scheduler.json`, adopts it and reports its real duration and exit code when it ends. Only the timer stops on SIGTERM - never the job.
- **A blocked queue stays noisy.** Waiting on the lock is re-logged every half hour, and after six hours - the point `runHyperopt.sh` used to give up at - the wait is recorded as a blocked entry on the Jobs page. A run that hangs alive while holding the lock (one did for two days in August 2026) must not be able to stop the site producing predictions with a single log line as the only evidence.
- **Every run is recorded**: job, reason it started, duration, exit code and the git SHA it ran at, on the Jobs page and in `log/scheduler.log`. A job that was killed without writing an exit code is recorded as that, never as a success.

Modes. `SCHEDULER=dry` (the default) records what it would have started without starting anything - that is what makes deploying this next to a live crontab safe; `SCHEDULER=on` runs the schedule; `SCHEDULER=off` disables it, as does `touch config/scheduler.disabled`, which takes effect without a restart and is the fallback lever. Rolling back is a real rollback: the queue survives in `config/scheduler.json` across a restart, so anything the schedule had queued is **discarded** when the mode is no longer `on` (a job started by hand is kept), rather than waiting to fire the next time the schedule comes up. *Run now*, *Cancel* and *Stop* on the Jobs page work in `dry` as well (they are explicit human actions; *Stop* signals the whole process group, so the Python process goes with its wrapper) and are admin-only, POST and CSRF-checked like everything else there. State lives in `config/scheduler.json` and the per-run files in `config/runs/`, gitignored like the accounts - `data/` is the pipeline's own, committed, territory.
