...
Pm2 can also be used. For this run (the ignore list keeps account changes and the daily data commits from restarting the server; pass the login variables through the environment or a pm2 ecosystem file):

```
    pm2 start server.js --name predictor --time --watch --ignore-watch="config data node_modules"
```
