...
- **`SubsetEnsemble Model`** (roadmap item 2) - the same vote as `WeightedEnsemble Model`, restricted to a *searched subset* of rows. `HyperoptEnsemble.py` selects which rows take part (one include flag per candidate row as Optuna parameters, plus a weighted-versus-flat vote choice) and writes the selection to `bestParams_<game>.json` (`subsetEnsembleModels`, `subsetEnsembleWeighted`); `addSubsetEnsemblePrediction` serves it with the shared recipe in `Helpers.build_vote_ensemble_predictions` (which `WeightedEnsemble Model` now uses too), so the served ticket is exactly what the tuner scored. Not served until a selection exists, and skipped on a day one of the selected rows did not run - a vote among the remaining rows would be a different ensemble under this row's name. Its own result is never written into `modelScores`, so it cannot feed back into its members' weights. Positional games are skipped like `WeightedEnsemble Model` (roadmap item 7).
...
Games are classified once as **positional** (`Helpers.is_positional_game`: Pick3 and Joker+ - drawn order kept, digits may repeat, per-position models, no set-style rows such as WeightedEnsemble/MarkovBayesian/multi-label boosting) or set-based; every positional branch keys on that predicate, not on a game name.
...
`HyperoptEnsemble.py` selects the rows of the `SubsetEnsemble Model` (roadmap item 2). Like the RL tuner it works on the stored day JSONs rather than the Backtester - the only place where every tracked row (statistical, boosting, deep learning, meta-learner, quantum) exists side by side for the same draws, tuned as it was at the time. Per game: the candidate rows are those present on at least 80% of the last `--days` (default 120) scoreable days, minus the two vote rows and the RL row (appended after the vote, so never available to it); one Optuna include flag per candidate plus `subsetEnsembleWeighted` (vote with the `modelScores` weights or count every member once); a trial votes the selected rows with `Helpers.build_vote_ensemble_predictions` on every day all of them exist and is scored like the report scores served rows - profit per bet over the enabled Keno subsets, main-ticket hits elsewhere. A subset is scored only on the days all of its members exist, and a trial counts only when that is at least 60% of the window - every scored trial then sits on nearly the same draws, which is what makes trial values comparable. This rule came out of testing: with a looser floor the search found a six-row subset that happened to break even on the ten days its members coexisted and ranked it above every subset that lost keno's house edge over 300 days - with thousands of subsets in the space such a fluke is always there to be found. The consequence is that the weeks-old boosting/meta-learner/quantum rows are not candidates yet at the default window; they join automatically as they age into it, or right away with a shorter window (`--days 60`), at the price of fewer draws for everyone. The trial value is a lower confidence bound on the per-day score, mean − std/√days (`--confidence-penalty`, default 1; 0 gives the plain mean), which mildly favours the subsets covering more of the window. Fewer than two members prunes the trial. The all-in weighted vote (the `WeightedEnsemble Model` over the same rows) is enqueued as the baseline every study starts from, so the selection can only improve on it. Trials take a couple of seconds (mostly process start-up), so the default is 200 per game, one at a time (`--parallel-trials`; the runner spaces launches 3 s apart, which would make parallel trials slower here, not faster). Only the derived selection is written to `bestParams_<game>.json` (`subsetEnsembleModels`, `subsetEnsembleWeighted`, `subsetEnsembleObjective`, `subsetEnsembleMean`, `subsetEnsembleTunedOn`) - never the twenty include flags, and never the row's own score into `modelScores`. The best trial is picked from the current run only, since each weekly run scores on a longer window. Runs in `runHyperopt.sh` after `HyperoptRLTicket.py`; positional games are skipped until roadmap item 7 gives them a per-slot vote. Honest caveat: the selection is made on the rows' stored real-life history, so on the day it is written the subset is in-sample by construction - what makes it a fair row is that its tracked results accrue only from then on, out of sample, next to every other row.
...
- **Searched subset-ensemble** - `SubsetEnsemble Model`, selected by the new `HyperoptEnsemble.py` (see *Hyperopt & backtesting*) and served by `Predictor.addSubsetEnsemblePrediction`; the vote recipe moved into `Helpers.build_vote_ensemble_predictions` so the served row and the tuner cannot drift apart. Not yet for pick3/Joker+ - that needs the per-slot vote of item 7, whose tuning study then lives in `HyperoptEnsemble.py` as well.

Open: the first selections are written by the next weekly `runHyperopt.sh`; until then the row is absent (a manual `python3 HyperoptEnsemble.py -g keno,lotto` writes them right away).
...
**Second foundation model to check: TimesFM-3.** Google's TimesFM (decoder-only, pretrained on a large corpus of real series; TimesFM-3 is the release to use) runs through exactly the same zero-shot per-position protocol - same folds, same order-statistics baseline, same negative-control reading - so the two models are compared on identical data rather than each against chance, tracked as its own row (`TimesFM Model`). Practicalities: weights from Hugging Face; run in the spawned DL child, one foundation model loaded at a time on the 6 GB card (CPU inference is acceptable for a few dozen short series per day); check the release's framework and pinned dependency versions before adding it next to torch/`chronos-forecasting`. If both rows agree on the lottery track (nothing beyond the order-statistics baseline) that is a strong negative result; if either shows something on the markets track, GARCH (item 4) is the first thing to beat before reading it as skill.
...
### 7. `WeightedEnsemble Model` for pick3 - a vote per position (medium)
