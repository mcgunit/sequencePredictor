...
- **XGBoost** (`src/XGBoost.py`) — gradient boosting, now a first-class model on the exact same interface as the statistical models rather than the loosely-wired "optional boosting pass" it used to be. It fits one multiclass classifier per draw position over a flattened window of the `xgBoostPreviousDraws` preceding draws; each position's class probabilities are averaged per number to produce both the ticket and a `score_numbers()` dict. It exposes `setDataPath` / `setSortedPrediction` / `clear` / `run(generateSubsets, skipRows, skipLastColumns, specialColumnCount, years_back)` / `score_numbers(...)`, so `Helpers.run_model_with_special_column`, `Backtester.py` and `Predictor.py` drive it exactly like `Markov` or `PoissonMarkov`. It runs for every game (`useBoost`) as its own tracked `XGBoost Model` row. What this alignment fixed along the way:
...
**Boosting / stacking:**
- **A second boosting method** — LightGBM or CatBoost as a separate tracked row. `HyperoptBoost.py`'s `STRATEGIES` table is already shaped for it (one entry per method, same as `HyperoptStatistics.py`), so the work is a new `src/` model on the shared interface, not pipeline plumbing.
- **Multi-label instead of per-position multiclass** — `XGBoost.py` fits one classifier per draw position, which for non-positional games (Keno's 20 unordered slots) models something the game doesn't actually have, and is why collision-refilling is needed to get a full ticket at all. A single multi-label / one-vs-rest fit over the number range would match the game's structure directly and cost far less to train.
