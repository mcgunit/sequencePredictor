...
def build_training_table(rows, model_names, min_number, max_number):
    """
    Turns Backtester rows (from a collect_scores=True backtest) into a flat
    supervised table: one row per (backtest day, number in game range), with
    a feature per base model's score for that number (0 if unscored) and a
    binary label for whether that number was actually drawn that day.
    """
    features = []
    labels = []
    ...
    for row in rows:
        ...
        for number in range(min_number, max_number + 1):
            feature = [row.get(f"{name}_scores", {}).get(number, 0.0) for name in model_names]
...
def train_meta_learner(dataset_name, game_cfg, path, days_back):
    ...
    # Backtester.backtest() preserves day order (pool.imap, not imap_unordered),
    # so a plain prefix/suffix split is a genuine walk-forward holdout - the
    # meta-learner is evaluated only on days after everything it trained on.
    split_index = int(len(results) * 0.8)
    train_rows, test_rows = results[:split_index], results[split_index:]

    X_train, y_train = build_training_table(train_rows, model_names, game_cfg["min"], game_cfg["max"])
    X_test, y_test = build_training_table(test_rows, model_names, game_cfg["min"], game_cfg["max"])
    ...
    if len(test_rows) > 0 and len(set(y_test.tolist())) > 1:
        ...
        print(f"{dataset_name}: held-out accuracy={accuracy_score(y_test, y_pred):.4f} "
              f"auc={roc_auc_score(y_test, y_proba):.4f} (train_days={len(train_rows)}, test_days={len(test_rows)})")
    else:
        print(f"{dataset_name}: not enough held-out days/classes to report accuracy/AUC "
              f"(train_days={len(train_rows)}, test_days={len(test_rows)})")

    # Refit on the full available window (train+test) before persisting, now
    # that the held-out score above has already given an honest read on it -
    # standard practice once you're ready to ship, not a leakage shortcut.
    X_full, y_full = build_training_table(results, model_names, game_cfg["min"], game_cfg["max"])
