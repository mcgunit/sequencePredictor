# UnifiedLstmGruTcn.py
# Same fusion approach as UnifiedLstmTcn.py, with a third GRU branch added
# for extra architectural diversity (see README's "Ideas worth researching
# further" - this and UnifiedLstmTcn.py are tracked as separate rows so
# real-life performance decides whether the extra GRU branch is worth its
# added training cost over the 2-branch version).
import os, sys, json
import pandas as pd
import numpy as np
...
# ---------------------------
# UnifiedLstmGruTcn Model Class
# ---------------------------
class UnifiedLstmGruTcnModel:
    def __init__(self):
        self.dataPath = ""
        self.modelPath = ""
        self.epochs = 1000
    ...
    def create_model(self, max_value, num_classes=50, model_path="", digitsPerDraw=3, specialColumnCount=0, specialNumClasses=0):
        ...
        gru_branch = layers.Dropout(self.dropout)(gru_branch)
        ...
        fused = layers.Concatenate(axis=-1)([lstm_branch, gru_branch, tcn_branch])
    ...
    def run(self, name="pick3", skipLastColumns=0, maxRows=0, skipRows=0, years_back=None, strict_val=True, specialColumnCount=0):
        ...
        # what makes each position's class index decode correctly.
...
if __name__ == "__main__":
    model = UnifiedLstmGruTcnModel()

    name = "pick3"
    path = os.getcwd()
    dataPath = os.path.join(os.path.abspath(os.path.join(path, os.pardir)), "test", "trainingData", name)
    modelPath = os.path.join(os.path.abspath(os.path.join(path, os.pardir)), "test", "models", "unified_lstm_gru_tcn_model")
